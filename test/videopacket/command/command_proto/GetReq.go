// Package hello comment
// This file war generated by trpc4videopacket 1.1.0
// Generated from hello.jce
package hello

import (
	"fmt"

	"git.code.oa.com/jce/jce"
)

// GetReq struct implement
type GetReq struct {
	A int32 `xml:"a" json:"a"`
	B int32 `xml:"b" json:"b"`
}

// ResetDefault ...
func (st *GetReq) ResetDefault() {
}

// ReadFrom reads  from _is and put into struct.
func (st *GetReq) ReadFrom(_is *jce.Reader) error {
	var err error
	var length int32
	var have bool
	var ty byte
	st.ResetDefault()

	err = _is.Read_int32(&st.A, 0, false)
	if err != nil {
		return err
	}

	err = _is.Read_int32(&st.B, 1, false)
	if err != nil {
		return err
	}

	_ = err
	_ = length
	_ = have
	_ = ty
	return nil
}

//ReadBlock reads struct from the given tag , require or optional.
func (st *GetReq) ReadBlock(_is *jce.Reader, tag byte, require bool) error {
	var err error
	var have bool
	st.ResetDefault()

	err, have = _is.SkipTo(jce.STRUCT_BEGIN, tag, require)
	if err != nil {
		return err
	}
	if !have {
		if require {
			return fmt.Errorf("require GetReq, but not exist. tag %d", tag)
		}
		return nil

	}

	st.ReadFrom(_is)

	err = _is.SkipToStructEnd()
	if err != nil {
		return err
	}
	_ = have
	return nil
}

//WriteTo encode struct to buffer
func (st *GetReq) WriteTo(_os *jce.Buffer) error {
	var err error
	_ = err

	err = _os.Write_int32(st.A, 0)
	if err != nil {
		return err
	}

	err = _os.Write_int32(st.B, 1)
	if err != nil {
		return err
	}

	return nil
}

//WriteBlock encode struct
func (st *GetReq) WriteBlock(_os *jce.Buffer, tag byte) error {
	var err error
	err = _os.WriteHead(jce.STRUCT_BEGIN, tag)
	if err != nil {
		return err
	}

	st.WriteTo(_os)

	err = _os.WriteHead(jce.STRUCT_END, 0)
	if err != nil {
		return err
	}
	return nil
}
